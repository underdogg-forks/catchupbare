...
@section('content')
    ...
    @if ($accGateway)
        {!! Former::populateField('primary_gateway_id', $accGateway->gateway_id) !!}
        {!! Former::populateField('recommendedGateway_id', $accGateway->gateway_id) !!}
        {!! Former::populateField('show_address', intval($accGateway->show_address)) !!}
        {!! Former::populateField('update_address', intval($accGateway->update_address)) !!}
        ...
        @if ($config)
            @foreach ($accGateway->fields as $field => $junk)
                @if (in_array($field, $hiddenFields))
                    {{-- do nothing --}}
                @elseif (isset($config->$field))
                    {{ Former::populateField($accGateway->gateway_id.'_'.$field, $config->$field) }}
                @endif
            @endforeach
        @endif
    @else
    ...
    @foreach ($gateways as $gateway)
        ...
        <div id="gateway_{{ $gateway->id }}_div" class='gateway-fields' style="display: none">
            @foreach ($gateway->fields as $field => $details)
                ...
                @if ($details && !$accGateway && !is_array($details))
            ...
            @if ($gateway->id == GATEWAY_STRIPE)
                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-8 col-sm-8 help-block">
                        <input type="text"  class="form-control" onfocus="$(this).select()" readonly value="{{ URL::to(env('WEBHOOK_PREFIX','').'payment_hook/'.$company->acc_key.'/'.GATEWAY_STRIPE) }}">
    ...
    @if (!$accGateway || $accGateway->gateway_id == GATEWAY_STRIPE)
    ...
    @elseif ($accGateway && $accGateway->gateway_id == GATEWAY_WEPAY)
            {!! Former::checkbox('enable_ach')
                        ->label(trans('texts.ach'))
                        ->text(trans('texts.enable_ach'))
                        ->value(1) !!}
    ...
    $(function() {
        ...
        @if (!$accGateway && count($secondaryGateways))
