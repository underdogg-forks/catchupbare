...
@section('content')
    ...
    <div class="panel-body">
            ...
            <div class="form-padding-right">
                ...
                @else
                    {!! Former::select('bank_id')
                            ...
                            ->blockHelp(trans('texts.bank_accs_help', ['link' => OFX_HOME_URL]))  !!}
                @endif
        ...
        <div class="col-lg-12 col-sm-12" data-bind="visible: page() == 'setup'" style="display:none">
            <table class="table companies-table" style="width:100%">
                <thead>
                    <tr>
                        <th class="header">{{ trans('texts.acc_name') }}</th>
                        <th class="header">{{ trans('texts.acc_number') }}</th>
                        <th class="header">{{ trans('texts.balance') }}</th>
                        <th class="header">{{ trans('texts.include') }}</th>
                    </tr>
                </thead>
                <tbody data-bind="foreach: bank_accs">
                    <tr>
                        <td>
                            <input type="text" class="form-control" data-bind="value: acc_name, valueUpdate: 'afterkeydown', attr: {name: 'bank_accs[' + $index() + '][acc_name]'}"/>
                            <input type="text" style="display:none" data-bind="value: hashed_acc_number, attr: {name: 'bank_accs[' + $index() + '][hashed_acc_number]'}"/>
                        </td>
                        <td data-bind="text: masked_acc_number"></td>
                        <td data-bind="text: balance"></td>
                        <td style="text-align:center">
                            <input type="checkbox" value="1"
                                data-bind="checked: includeAccount, attr: {name: 'bank_accs[' + $index() + '][include]'}"/>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="col-lg-12 col-sm-12" data-bind="visible: page() == 'import'" style="display:none">
            ...
            <div data-bind="foreach: bank_accs">

                <h4 data-bind="text: acc_name"></h4><br/>
                ...
                <table class="table companies-table" style="width:100%;">
                    ...
                    <tbody data-bind="foreach: filteredTransactions">
                        <tr>
                            <td style="text-align:center">
                                <input type="checkbox" value="1"
                                    data-bind="checked: includeTransaction, attr: {name: 'bank_accs[' + $index() + '][include]'}"/>
                            </td>
                            <td>
        ...
        {!! Former::actions(
            count(Cache::get('banks')) > 0 ?
                Button::normal(trans('texts.cancel'))
                    ...
                    ->large()
                    ->asLinkTo(URL::to('/settings/bank_accs'))
    ...
    function validate() {
        ...
        $.post('{{ URL::to('/bank_accs/validate') }}', $('.main-form').serialize())
            .done(function(data) {
    ...
    function save() {
        model.errorStr(false);
        model.isLoading(true);
        $.post('{{ URL::to('/bank_accs') }}', $('.main-form').serialize())
            .done(function(data) {
                data = JSON.parse(data);
                if (!data || !data.length) {
                    model.isLoading(false);
    ...
    function loadTransactions(data) {
        model.bank_accs.removeAll();
        for (var i=0; i<data.length; i++) {
            var row = data[i];
            var company = new BankAccountModel(row);
            if (row.transactions) {
                var transactions = company.transactions();
                ...
                company.transactions.valueHasMutated();
            }
            model.bank_accs.push(company);
        }
    }
    ...
    function saveExpenses() {
        ...
        $.ajax({
            url: '{{ URL::to('/bank_accs/import_expenses') }}' + '/' + model.bank_id(),
            type: 'POST',
    ...
    var TransactionModel = function(data) {
        ...
        self.vendor.pretty = ko.computed({
            ...
            write: function (value) {
                this.vendor(value);
                for (var i=0; i<model.bank_accs().length; i++) {
                    var company = model.bank_accs()[i];
                    var transactions = company.transactions();
    ...
    var BankAccountModel = function(data) {
        ...
        self.hashed_acc_number = ko.observable(data.hashed_acc_number);
        self.masked_acc_number = ko.observable(data.masked_acc_number);
        self.acc_name = ko.observable(data.acc_name);
        ...
        self.isValid = ko.computed(function() {
            return self.acc_name() ? true : false;
        }, self);
    ...
    var ViewModel = function() {
        ...
        self.bank_accs = ko.observableArray();
        ...
        self.setPage = function(page) {
            ...
            if (page == 'login') {
                ...
                @else
                    self.title("{{ trans('texts.add_bank_acc') }}");
                @endif
            } else if (page == 'setup') {
        ...
        self.includedTransactions = ko.computed(function() {
            var data = [];
            for (var i=0; i<self.bank_accs().length; i++) {
                var company = self.bank_accs()[i];
                var transactions = ko.utils.arrayFilter(company.transactions(), function(transaction) {
