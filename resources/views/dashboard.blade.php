                                ...
                                <div class="in-bold">
                                    @if (count($paidToDate))
                                        @foreach ($paidToDate as $item)
                                            <div class="currency currency_{{ $item->currency_id ?: $company->getCurrencyId() }}"
                                                 style="display:none">
                                                {{ Utils::formatMoney($item->value, $item->currency_id) }}
                                            </div>
                                        @endforeach
                                    @else
                                        <div class="currency currency_{{ $company->getCurrencyId() }}"
                                             style="display:none">
                                            {{ Utils::formatMoney(0) }}
                                        </div>
                                    @endif
                            ...
                            <h3 class="panel-title" style="color: black !important">
                                @if (count($expenses) && count($averageInvoice))
                                    <div class="pull-right" style="font-size:14px;padding-top:4px;font-weight:bold">
                                        @foreach ($averageInvoice as $item)
                                            <span class="currency currency_{{ $item->currency_id ?: $company->getCurrencyId() }}"
                                                  style="display:none">
                                    {{ trans('texts.average_invoice') }}
                                                {{ Utils::formatMoney($item->invoice_avg, $item->currency_id) }} |
                                </span>
